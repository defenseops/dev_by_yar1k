...
const colors = ['#818CF8','#FBBF24','#34D399','#F472B6','#A78BFA','#60A5FA']
      ...
      <div className="container relative">
        <div className="grid lg:grid-cols-2 gap-12 mb-14">
          <motion.div initial={{ opacity:0, y:24 }} whileInView={{ opacity:1, y:0 }} viewport={{ once:true }} transition={{ duration:0.55 }}>
            ...
            <h2 className="h2 text-white whitespace-pre-line">{d.title}</h2>
          </motion.div>
          <motion.p initial={{ opacity:0, y:20 }} whileInView={{ opacity:1, y:0 }} viewport={{ once:true }}
            transition={{ duration:0.5, delay:0.1 }}
            className="text-white/40 leading-relaxed self-end text-lg">{d.desc}</motion.p>
        </div>
          ...
          {d.items.map(({ title, body }, i) => {
            ...
            return (
              <motion.div key={title}
                ...
                <h3 className="font-bold text-white mb-2 group-hover:text-brand transition-colors duration-200">{title}</h3>
